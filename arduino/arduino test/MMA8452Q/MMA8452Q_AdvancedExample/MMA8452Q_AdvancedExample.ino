...
const byte fsr = 2;  // Sets full-scale range to +/-2, 4, or 8g. Used to calc real g values.
...
void setup()
{
  ...
  {  
    initMMA8452(fsr, dataRate);  // init the accelerometer if communication is OK
...
{  
  ...
  if (digitalRead(int1Pin)==1)  // Interrupt pin, should probably attach to interrupt function
  {
    ...
    for (int i=0; i<3; i++)
      accelG[i] = (float) accelCount[i]/((1<<12)/(2*fsr));  // get actual g value, this depends on scale being set
    // Print out values
    for (int i=0; i<3; i++)
    {
